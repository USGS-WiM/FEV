...
function getSofarData(type, name, url, markerIcon) {
  let sofarBouyIds = ['30080D', '30082D', '30083D']
  var sofarmarkers = {};
  for (var i = 0; i < sofarBouyIds.length; i++) {
    ...
    $.ajax({
      ...
      dataType: "json",
      id: id,
      headers: { token: '95db5defab9744c20a78ceeea6ea2e' }, // remove token id before pushing to github
...
function querySofarGraph(e) {
  ...
  e.layer
    .bindPopup(
      ...
      "SOFAR" +
      ' | <span style="color:gray">' +
      e.layer.data.buoyID + ' | ' + '<a href="https://watp.sofarocean.com/historical/SPOT-' + e.layer.data.id +'" target="_blank"> Sofar Dashboard</a>' +
