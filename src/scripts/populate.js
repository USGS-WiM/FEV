...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        $("#evtTypeSelect").append(
          '<option value="' +
            data[i].event_type_id +
            '">' +
            data[i].type +
            "</option>"
        );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        data[i].id = data[i].event_id;
        fev.dataCompare.events.push(data[i]);
      }
    },
    error: function (error) {
      console.log("Error processing the JSON. The error is:" + error);
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        data[i].id = data[i];
        fev.dataCompare.states.push(data[i]);
      }
      populateCountiesArrayCompare();
    },
    error: function (error) {
      console.log("Error processing the JSON. The error is:" + error);
    },
  ...
  //county select is a special case - populated values depend on states selected. see other logic TBD
  $("#countySelectCompare").select2({
    placeholder: "All Counties",
  });

  $("#countySelectCompare").on(
    "select2:select select2:unselect",
    function (selection) {
      //will need special treatment for display string creation
    }
  ...
  $("#sensorTypeSelectCompare").select2({
    placeholder: "All Types",
  });
  $.ajax({
    dataType: "json",
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        fev.dataCompare.sensorTypes.push(data[i]);
      }
    },
    error: function (error) {
      console.log("Error processing the JSON. The error is:" + error);
  ...
  $.ajax({
    ...
    success: function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#sensorStatusSelect").append(
          '<option value="' +
            data[i].status_type_id +
            '">' +
            data[i].status +
            "</option>"
        );
  ...
  $("#sensorStatusSelectCompare").select2({
    placeholder: "All Statuses",
  });
  $.ajax({
    dataType: "json",
    ...
    success: function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#sensorStatusSelectCompare").append(
          '<option value="' +
            data[i].status_type_id +
            '">' +
            data[i].status +
        ...
        fev.dataCompare.sensorStatusTypes.push(data[i]);
      }
    },
    error: function (error) {
      console.log("Error processing the JSON. The error is:" + error);
  ...
  $.ajax({
    ...
    success: function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#collectionConditionSelect").append(
          '<option value="' +
            data[i].id +
            '">' +
            data[i].condition +
  ...
  $("#collectionConditionSelectCompare").select2({
    placeholder: "All Conditions",
  });
  $.ajax({
    dataType: "json",
    ...
    success: function (data) {
      for (var i = 0; i < data.length; i++) {
        $("#collectionConditionSelectCompare").append(
          '<option value="' +
            data[i].id +
            '">' +
            data[i].condition +
        ...
        fev.dataCompare.collectionConditions.push(data[i]);
      }
    },
    error: function (error) {
      console.log("Error processing the JSON. The error is:" + error);
  ...
  $("#deployTypeSelectCompare").select2({
    placeholder: "All Deploy Types",
  });
  $.ajax({
    dataType: "json",
    ...
    success: function (data) {
      for (var i = 0; i < data.length; i++) {
        ...
        fev.dataCompare.deploymentTypes.push(data[i]);
      }
    },
    error: function (error) {
      console.log("Error processing the JSON. The error is:" + error);
  ...
  var populateCountiesArray = function () {
    for (var i = 0; i < fev.data.states.length; i++) {
      $.ajax({
        ...
        type: "GET",
        url:
          "https://stn.wim.usgs.gov/STNServices/Sites/CountiesByState.json?StateAbbrev=" +
          fev.data.states[i].state_abbrev,
        headers: { Accept: "*/*" },
  ...
  var populateCountiesArrayCompare = function () {
    for (var i = 0; i < fev.dataCompare.states.length; i++) {
      $.ajax({
        dataType: "json",
        type: "GET",
        url:
          "https://stn.wim.usgs.gov/STNServices/Sites/CountiesByState.json?StateAbbrev=" +
          fev.dataCompare.states[i].state_abbrev,
        headers: { Accept: "*/*" },
        currentState: fev.dataCompare.states[i].state_abbrev,
        success: function (data) {
          fev.dataCompare.counties[this.currentState] = data;
          //console.log("Loaded counties for: ",this.currentState)
        },
        error: function (error) {
          console.log("Error retrieving counties. The error is: " + error);
  ...
  //begin onChange function for event compare state form (updates county options based on state selection)
  $("#stateSelectCompare").on("select2:select select2:unselect", function () {
    var currentSelection = $(this).val();
    if (!currentSelection > 0 || currentSelection === null) {
      $("#countySelectCompare").html("");
      $("#countySelectCompare").append(
        "<option value=null>Please select state(s) first </option>"
      );
      return;
    }
    var currentCounties = [];
    for (var key in fev.dataCompare.counties) {
      for (var i = 0; i < fev.dataCompare.counties[key].length; i++) {
        var value = fev.dataCompare.counties[key][i].county_name;
        if (currentSelection.indexOf(key) > -1) {
          currentCounties = currentCounties.concat(value);
        }
      }
  ...
  //end onChange function for event compare state form
});
